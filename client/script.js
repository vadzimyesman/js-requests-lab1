...
// CODE HERE
const changeBackgroundColorToBlack=(event)=>{
    event.target.classList.add("backgroundBlack")
}

sayHelloButton.addEventListener("mouseover",changeBackgroundColorToBlack)
...
// CODE HERE
const changeBackgroundColorToDefault=(event)=>{
    event.target.classList.remove("backgroundBlack")
}

sayHelloButton.addEventListener("mouseout",changeBackgroundColorToDefault)
...
/*
    ...
    Handle the promise returned from the request with a .then, which will take in a callback -- the callback function should print 
    the response.data.
